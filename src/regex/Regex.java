...
class Regex {
    public static void main(String[] args) {
        ...
        System.out.println("The following pattern ('\\d'){36} matches : the first sequence of exactly 36 digits.");
        ...
        Pattern pattern = Pattern.compile(regex);
            for(int i = 0; i < TEKmentors.length; i++) {
                String fullName = TEKmentors[i];
                Matcher matcher = pattern.matcher(fullName);
                matcher.find();
                String firstName = matcher.group();
                tekMentors[i] = firstName;
        }
        System.out.println(Arrays.toString(tekMentors));

        // 3. Find all the occurences of any form of 'book' in the following paragraph. use regex to match the
        // occurences and store the count  of books in an int.
        
        int bookCount = 0;
        String bookText = "Books are the keys to knowledge.  I didn't like to read books as a child, but in college I" +
                " started enjoying learning  and reading books. You can borrows books from the library, or you can " +
                "buy them from the bookstore.  I'm not sure if I prefer paperback books or hardcover books.  I'm such" +
                " a nerd that I even  like textbooks.  With the advent of technology, you can even buy digital books," +
                " such as kindle-books,  nook-books, or other e-books. My personal favorite book format are " +
        ...
        String regex2 = "book";
        Pattern bookPattern = Pattern.compile(regex2, Pattern.CASE_INSENSITIVE);
        Matcher matcher = bookPattern.matcher(bookText);
        while(matcher.find()) {
            bookCount++;
        }
        ...
        String not_sleepy = String.join("", noSleepy);; //punctuation marks will be here
